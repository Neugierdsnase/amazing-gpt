...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  return `<div class="flex flex-col gap-8 min-h-screen items-stretch my-12 pb-12"><div class="p-4 md:p-12 flex flex-wrap gap-4 items-center flex-col md:flex-row-reverse justify-between rounded-xl bg-base-300"><div class="flex justify-center items-center h-44 w-44"><img${add_attribute("src", image, 0)}${add_attribute("alt", displayname, 0)} class="mask mask-circle w-full"></div> <div><h1 class="text-4xl font-bold">${escape(displayname)}</h1> ${validate_component(AuthorSpan, "AuthorSpan").$$render($$result, { authorname, authorurl }, {}, {})} ${validate_component(Tags, "Tags").$$render($$result, { size: "sm", tags }, {}, {})}</div></div> <a href="${escape(BASE_OPENAI_GPT_URL, true) + "/" + escape(slug, true)}" class="btn btn-secondary md:w-1/3 btn-lg self-center" target="_blank" rel="noopener noreferrer">Use
