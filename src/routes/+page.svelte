<script lang="ts">
	...
	import GptList from '$lib/components/GPTList/GPTList.svelte';
	...
	$: gpts = derived([gptStore, filterStore], ([$gptStore, $filterStore]) => {
		...
		if (tags.length > 0) {
			filteredStore = filteredStore.filter((gpt) => {
				return tags.every((tag) => gpt.tags.includes(tag));
			});
		}
...
<h1
	class="text-9xl font-extrabold mx-auto px-8 text-center py-8 masked-text"
...
>
	A curated list of amazing custom GPTs
</h1>
<FilterBar {tags} activeTags={$filterStore.tags} />
<GptList items={$gpts} />

<style>
	.masked-text {
		@apply relative text-transparent bg-clip-text bg-secondary;
